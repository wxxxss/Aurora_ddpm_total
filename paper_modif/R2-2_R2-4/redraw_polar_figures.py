...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "Display-only floor for reconstruction/OVATION panels. Flux below this "
            "value is transparent; source arrays and quantitative results are unchanged."
        ),
    )
...
def validate_panel_shapes(observation: np.ndarray, reconstruction: np.ndarray, ovation: np.ndarray) -> None:
    ...
    if len(set(shapes)) != 1:
        raise ValueError(
            f"Panel shape mismatch: observation={shapes[0]}, reconstruction={shapes[1]}, "
            f"ovation={shapes[2]}"
        )
...
def prepare_observation_for_plot(
    flux: np.ndarray,
    support_threshold: float = OBS_SUPPORT_THRESHOLD,
) -> np.ndarray:
    """Return a display copy using the legacy Polar support rule (flux >= 1)."""
...
def prepare_full_field_for_plot(
    ...
    """Return a display-only copy with near-zero background made transparent."""
    out = np.asarray(flux, dtype=np.float32).copy()
    out[(~np.isfinite(out)) | (out < display_floor)] = np.nan
    return out


def panel_stats(flux: np.ndarray, threshold: float | None = None) -> Dict[str, float]:
    ...
    if finite.size == 0:
        return {
            ...
            "p99": float("nan"),
            "max": float("nan"),
            "fraction_below_threshold": float("nan"),
        }
    stats = {
    ...
    if threshold is None:
        stats["fraction_below_threshold"] = float("nan")
    else:
        stats["fraction_below_threshold"] = float(np.mean(finite < threshold))
    return stats
...
def match_event(
    ...
    repaired_data: np.ndarray,
    omni_data: np.ndarray,
...
def make_aurora_cmap() -> LinearSegmentedColormap:
    ...
    # Fully transparent masked cells.  Black RGB avoids a white fringe on
    # backends that pre-multiply RGB before applying alpha.
...
def plot_flux_on_axis(
    ...
    masked = np.ma.masked_where(~np.isfinite(flux), np.asarray(flux, dtype=np.float32))
...
def event_title(timestamp: datetime) -> str:
    return f"Polar/UVI -- {timestamp.strftime('%-d %B %Y')} -- {timestamp.strftime('%H:%M UT')}"
...
def plot_event(
    ...
    panels = [
        ...
        prepare_full_field_for_plot(case["reconstruction"], display_floor=display_floor),
        prepare_full_field_for_plot(case["ovation"], display_floor=display_floor),
    ]
    titles = [
    ...
    fig.subplots_adjust(left=0.015, right=0.985, top=0.91, bottom=0.13, wspace=0.035)
...
def load_inputs(args: argparse.Namespace):
    for label, path in {
        "Polar/UVI": args.polar_data,
        "saved reconstruction": args.repaired_data,
        "OVATION": args.ovation_data,
        "OMNI": args.omni_data,
    }.items():
        if not path.exists():
...
def main() -> None:
    ...
    print("=" * 88)
    print(f"Display floor for reconstruction/OVATION: {args.display_floor:.3f}")
    ...
    for case_name, target in EVENTS.items():
        case = match_event(
            ...
            omni_data,
            ovation_data,
            args.max_delta_seconds,
        )
        out = args.output_root / f"{case_name}_Polar_UVI_manuscript.png"
        plot_event(case, out, args.background, args.vmax, args.display_floor)

        stats = {
            "observation": panel_stats(case["observation"], OBS_SUPPORT_THRESHOLD),
            "reconstruction": panel_stats(case["reconstruction"], args.display_floor),
            "ovation": panel_stats(case["ovation"], args.display_floor),
        }
        ...
        print(
            f"  matched Polar={case['polar_time']} recon={case['repaired_time']} "
        ...
        for panel_name in ("observation", "reconstruction", "ovation"):
            ...
            print(
                ...
                f"below-threshold={s['fraction_below_threshold']:.3f}"
            )
