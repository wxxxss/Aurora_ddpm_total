...
class DDPM_nocond(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        return self.generator(x, timesteps)
    ...
    def get_inference_schedule(
        ...
        ts.append(-1)
        return ts
    def step(self, timestep, xt_image, x0_image, mask, guidance_scale=2):
        # 🔧 修复1：确保timestep是整数类型
        ...
        # 模型预测
        epsilon_theta = self(x_current_t, timestep)
    ...
    def sample(self, image, mask, num_inference_steps=250, n_sample=1, j=10, r=10):
        ...
        for t_last, t_cur in zip(times[:-1], times[1:]):
            if t_cur < t_last:  # 反向
                ...
                x_t = self.step(t_tensor, x_t, image, mask)
